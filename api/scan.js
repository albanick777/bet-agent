  ...
  try {
    ...
    async function kvGet(key) {
      ...
      try {
        ...
        return typeof outer.value === "string" ? JSON.parse(outer.value) : outer;
      } catch { return null; }
    }
    ...
    // Guard — dacă scanarea a fost deja trimisă, nu mai trimite mesaj
    const scanDate = await kvGet("pattern_scan_date");
    const todayDate = new Date().toISOString().slice(0, 10);
    const alreadyScanned = scanDate === todayDate;
    ...
    for (const team of TEAMS) {
      try {
        ...
        finished.forEach(m => {
          ...
          htftMap[`${htResult}/${ftResult}`] = (htftMap[`${htResult}/${ftResult}`] || 0) + 1;
        });
    ...
    await kvSet("pattern_scan_date", todayDate);

    // Trimite mesaj Telegram doar dacă nu a fost trimis azi
    if (!alreadyScanned) {
      const sep = "━━━━━━━━━━━━━━━━━━━━";
      let message = `🔍 SCANARE PATTERN HT/FT\n${sep}\n`;
      message += `📅 ${todayDate}\n`;
      message += `✅ Echipe monitorizate: ${allPatternTeams.length}\n\n`;
      ...
      message += `${sep}\n🧠 Aceste echipe vor fi monitorizate zilnic.`;
