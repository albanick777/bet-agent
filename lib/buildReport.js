export async function buildEliteReport(lang = "ro", apiKey) {
  ...
  async function analyzeMatch(match) {
    ...
    const poisson = computePoissonPredictions(
      hS.attackStrength || 1, hS.defenseWeakness || 1,
      aS.attackStrength || 1, aS.defenseWeakness || 1, 2.7
    );
    ...
    if (r1Score >= MIN_CONFIDENCE && r1Classic >= 65) {
      picks.push(buildPick({
    ...
    // ── OVER 1.5 REPRIZA 1 ────────────────────────────────────────────────
    const r1Over15Classic = (hS.scoredR1Pct + aS.scoredR1Pct) / 2;
    const r1Over15Score = combinedConfidence(
      poisson.firstHalfOver15Pct, r1Over15Classic, 55, avgSP
    );
    if (r1Over15Score >= MIN_CONFIDENCE && r1Over15Classic >= 70 && poisson.firstHalfOver15Pct >= 45) {
      picks.push(buildPick({
        ...base, market: "OVER 1.5 R1", score: r1Over15Score, type: "FIRST_HALF",
        reason: `Poisson Over1.5 R1: ${poisson.firstHalfOver15Pct}% | Classic: ${r1Over15Classic.toFixed(0)}%`
      }));
    }
    ...
    const oneXScore = combinedConfidence(
      ...
      predHome + (predHome * 0.3), homePen + homeSP
    );
    ...
    const x2Score = combinedConfidence(
      ...
      predAway + (predAway * 0.3), awayPen + awaySP
    );
    ...
    if (hS.topHtftCode && hS.topHtftPct >= 55) {
      const htftScore = combinedConfidence(
        hS.topHtftPct, hS.topHtftPct, predHome * 0.5, homePen + homeSP
      );
    ...
    try {
      const homeFixtureIds = homeHistory
        .filter(m => ["FT","AET","PEN"].includes(m?.fixture?.status?.short || ""))
        .slice(0, 3).map(m => m?.fixture?.id).filter(Boolean);
...
const top5 = allPicks.filter(p => {
  ...
  // ─── ACUMULATOR SUGERAT ───────────────────────────────────────────────────
  // Picks compatibile — nu combinăm picks contradictorii din același meci
  const INCOMPATIBLE = [
    ["OVER 2.5", "UNDER 2.5"],
  ...
  if (accumulator.length >= 2) {
    ...
    // Cotă estimată simplă: dacă confidence e 85% → cota e ~1/0.85 = 1.18
