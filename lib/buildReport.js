export async function buildEliteReport(lang = "ro", apiKey) {
  ...
  const NATIONAL_LEAGUES = [
    { name: "Premier League", country: "England", code: "ENG1", avgGoals: 2.8 },
    { name: "La Liga", country: "Spain", code: "ESP1", avgGoals: 2.7 },
    { name: "Serie A", country: "Italy", code: "ITA1", avgGoals: 2.6 },
    { name: "Bundesliga", country: "Germany", code: "GER1", avgGoals: 3.1 },
    { name: "Ligue 1", country: "France", code: "FRA1", avgGoals: 2.6 },
    { name: "Eredivisie", country: "Netherlands", code: "NED1", avgGoals: 3.2 },
    { name: "Jupiler Pro League", country: "Belgium", code: "BEL1", avgGoals: 2.9 },
    { name: "Primeira Liga", country: "Portugal", code: "POR1", avgGoals: 2.6 },
    { name: "Super Lig", country: "Turkey", code: "TUR1", avgGoals: 2.7 },
    { name: "Scottish Premiership", country: "Scotland", code: "SCO1", avgGoals: 2.7 },
    { name: "Super League", country: "Switzerland", code: "SUI1", avgGoals: 2.8 },
    { name: "Super League 1", country: "Greece", code: "GRE1", avgGoals: 2.5 }
  ];

  const CUP_LEAGUES = [
    { name: "UEFA Champions League", country: "World", code: "UCL", avgGoals: 2.9 },
    { name: "UEFA Europa League", country: "World", code: "UEL", avgGoals: 2.7 },
    { name: "UEFA Europa Conference League", country: "World", code: "UECL", avgGoals: 2.6 }
  ];
  ...
  function getLeagueAvgGoals(leagueName, countryName) {
    const league = ALL_APPROVED.find(l => {
      const nameMatch = leagueName.toLowerCase() === l.name.toLowerCase();
      const countryMatch = l.country === "World" ||
        (countryName || "").toLowerCase() === l.country.toLowerCase();
      return nameMatch && countryMatch;
    });
    return league?.avgGoals || 2.7;
  }
  ...
  function computeELO(matches, teamName) {
    ...
    sorted.forEach(m => {
      ...
      const myGoals = isHome ? Number(m?.goals?.home ?? 0) : Number(m?.goals?.away ?? 0);
      const oppGoals = isHome ? Number(m?.goals?.away ?? 0) : Number(m?.goals?.home ?? 0);
  ...
  function computePoissonPredictions(homeAttack, homeDefense, awayAttack, awayDefense, leagueAvgGoals) {
    const homeLambda = Math.max(0.3, homeAttack * awayDefense * leagueAvgGoals);
    const awayLambda = Math.max(0.3, awayAttack * homeDefense * leagueAvgGoals);
    ...
    const firstHalfOver15 = 1 - poissonProb(firstHalfLambda, 0) - poissonProb(firstHalfLambda, 1);
    ...
    return {
      ...
      firstHalfOver15Pct: Math.round(firstHalfOver15 * 100),
  ...
  function computeFormIndex(matches, teamName) {
  ...
  function summarize(matches, teamName, sideFilter) {
    ...
    let over25=0, btts=0, under35=0, win=0, score2H=0, scoredR1=0, htGoal=0;
    const htftMap = {};
    ...
    return {
      ...
      attackStrength: Number((avgGF / (leagueAvg / 2)).toFixed(3)),
      defenseWeakness: Number((avgGA / (leagueAvg / 2)).toFixed(3)),
  ...
  async function getCornersAvg(fixtureIds) {
    ...
    for (const r of results) {
      ...
      for (const teamStats of stats) {
        const cornerStat = safeArray(teamStats?.statistics).find(s => s?.type === "Corner Kicks");
        if (cornerStat?.value != null) matchCorners += Number(cornerStat.value) || 0;
      }
  ...
  async function analyzeMatch(match) {
    ...
    const leagueAvgGoals = getLeagueAvgGoals(leagueName, countryName);
    ...
    const homeForm = computeFormIndex(homeHistory, home);
    const awayForm = computeFormIndex(awayHistory, away);
    ...
    // ── CONFIDENCE RECALIBRAT ─────────────────────────────────────────────
    // Ia cel mai bun semnal ca bază, nu media — mai realist
    function combinedConfidence(poissonPct, classicPct, apiPct, penalty) {
      const best = Math.max(poissonPct, classicPct);
      const worst = Math.min(poissonPct, classicPct);
      const raw = (best * 0.55) + (worst * 0.30) + (apiPct * 0.15);
    ...
    if (over25Score >= MIN_CONFIDENCE && poisson.expectedGoals >= 2.4) {
    ...
    const under25Score = combinedConfidence(
      under25Poisson,
      under25Classic,
      100 - Math.max(predHome, predAway),
      avgSP
    );
    ...
    // ── OVER 0.5 R1 ───────────────────────────────────────────────────────
    const r1Classic = (hS.htGoalPct + aS.htGoalPct) / 2;
    const r1Score = combinedConfidence(
      poisson.firstHalfGoalPct,
      r1Classic,
      65,
      avgSP
    );
    if (r1Score >= MIN_CONFIDENCE && r1Classic >= 60 && poisson.firstHalfGoalPct >= 70) {
      picks.push(buildPick({
        ...base, market: "OVER 0.5 R1", score: r1Score, type: "FIRST_HALF",
        reason: `Poisson R1: ${poisson.firstHalfGoalPct}% | Classic R1: ${r1Classic.toFixed(0)}%`
      }));
    }
    ...
    if (oneXScore >= MIN_CONFIDENCE && eloDiff >= 30) {
      picks.push(buildPick({
        ...base, market: "1X", score: oneXScore, type: "SAFE",
        reason: `Poisson 1X: ${homeWinPoisson}% | ELO: +${eloDiff} | Form: ${homeForm} vs ${awayForm}`
      }));
    }
    ...
    if (x2Score >= MIN_CONFIDENCE && eloDiff <= -30) {
      picks.push(buildPick({
        ...base, market: "X2", score: x2Score, type: "SAFE",
        reason: `Poisson X2: ${awayWinPoisson}% | ELO: ${eloDiff} | Form: ${awayForm} vs ${homeForm}`
      }));
    }
    ...
    // ── CORNERE DINAMIC — FILTRU DUBLU ────────────────────────────────────
    try {
      const homeFixtureIds = homeHistory
        .filter(m => ["FT","AET","PEN"].includes(m?.fixture?.status?.short || ""))
        .slice(0, 10).map(m => m?.fixture?.id).filter(Boolean);
      ...
      if (homeCorners !== null && awayCorners !== null) {
        ...
        let cornerMarket = null, cornerConf = 0;

        if (homeCorners >= 10 && awayCorners >= 10 && avgCorners >= 11) {
          cornerMarket = "CORNERE OVER 9.5";
          cornerConf = 86;
        } else if (homeCorners >= 9.5 && awayCorners >= 9.5 && avgCorners >= 10) {
          cornerMarket = "CORNERE OVER 9.5";
          cornerConf = 83;
        } else if (homeCorners >= 8.5 && awayCorners >= 8.5 && avgCorners >= 9) {
          cornerMarket = "CORNERE OVER 8.5";
          cornerConf = 81;
        } else if (homeCorners <= 6.5 && awayCorners <= 6.5 && avgCorners <= 6.5) {
          cornerMarket = "CORNERE UNDER 7.5";
          cornerConf = 82;
        } else if (homeCorners <= 7 && awayCorners <= 7 && avgCorners <= 7) {
          cornerMarket = "CORNERE UNDER 8.5";
          cornerConf = 80;
        }

        if (cornerMarket && (cornerConf - avgSP) >= MIN_CONFIDENCE) {
          picks.push(buildPick({
            ...base,
            market: cornerMarket,
  ...
  function isValidFixture(match) {
    ...
    if (!getApprovedLeague(match?.league?.name || "", match?.league?.country || "")) return false;
    const kickoffTs = match?.fixture?.timestamp;
    if (kickoffTs) {
  ...
  for (const match of nationalFixtures.slice(0, 15)) {
    try {
  ...
  if (allPicks.filter(p => p.confidence >= 80).length < 3) {
    for (const match of cupFixtures.slice(0, 8)) {
      try {
        const picks = await analyzeMatch(match);
        allPicks.push(...picks);
        analyzedCount.total++;
  ...
  function areCompatible(p1, p2) {
    if (p1.match === p2.match) {
      for (const [a, b] of INCOMPATIBLE) {
        if ((p1.market === a && p2.market === b) ||
            (p1.market === b && p2.market === a)) return false;
      }
    }
    return true;
  }
  ...
  for (const pick of top5) {
    ...
    if (accumulator.every(p => areCompatible(p, pick))) {
      accumulator.push(pick);
    }
  }

  let accumulatorSuggestion = null;
  if (accumulator.length >= 2) {
    const combinedPct = Math.round(
      accumulator.reduce((acc, p) => acc * (p.confidence / 100), 1) * 100
    );
    const estimatedOdds = accumulator
      .reduce((acc, p) => acc * (100 / p.confidence), 1)
      .toFixed(2);
    accumulatorSuggestion = { picks: accumulator, combinedPct, estimatedOdds };
  }
  ...
  for (const team of PATTERN_TEAMS) {
    ...
    try {
      ...
      patternWatch.push({
        ...
        league: leagueName, elo, stats, topSignal: signals
      });
    } catch (err) {
