export async function buildEliteReport(lang = "ro", apiKey) {
  ...
  function computeELO(matches, teamName) {
    let elo = 1500; // rating initial standard
    const K = 32;   // factor de ajustare
  ...
  function computePoissonPredictions(homeAttack, homeDefense, awayAttack, awayDefense, leagueAvgGoals = 2.7) {
    // Lambda = putere atac echipă × slăbiciune apărare adversar × medie ligă
    ...
    // Calculăm probabilități pentru 0-6 goluri per echipă
    for (let h = 0; h <= 6; h++) {
      for (let a = 0; a <= 6; a++) {
    ...
    // First half goal — aproximat ca 55% din goluri vin în prima repriză
    const expectedTotalGoals = homeLambda + awayLambda;
    const firstHalfLambda = expectedTotalGoals * 0.45;
    firstHalfGoal = 1 - poissonProb(firstHalfLambda, 0);
  ...
  async function analyzeMatch(match) {
    ...
    const eloHomeAdv = Math.round(50 + eloDiff / 20); // % avantaj ELO
    ...
    // ─── CONFIDENCE COMBINAT ───────────────────────────────────────────────
    // Combinăm Poisson (40%) + Statistici clasice (35%) + API predicție (25%)
    ...
    const picks = [];
    const MIN_CONFIDENCE = 80; // Prag ridicat la 80%
    ...
    // ── GOL REPRIZA 1 ─────────────────────────────────────────────────────
    const r1Classic = (hS.scoredR1Pct + aS.scoredR1Pct) / 2;
    ...
    if (r1Score >= MIN_CONFIDENCE) {
      picks.push(buildPick({
        ...base,
        market: "GOL REPRIZA 1",
        score: r1Score,
        type: "TIMING",
    ...
    const homeWinClassic = hS.winPct;
    ...
    const awayWinClassic = aS.winPct;
    ...
    // ── CORNERE OVER ──────────────────────────────────────────────────────
    // Vom adăuga date de cornere când implementăm endpoint-ul specific
    // Deocamdată placeholder pregătit pentru extensie
