...
const filteredFixtures = allFixtures.filter(match => {
  ...
  if (banned.some(b => league.includes(b))) return false;

  return allowedLeagues.some(l => league.includes(l));
});

  const upcoming = filteredFixtures.filter((match) => {
 ...
 const premiumPick = allPicks[0] || null;
const smartCombo = allPicks.slice(0, Math.min(3, allPicks.length));
const highCombo = allPicks.slice(0, Math.min(5, allPicks.length));
const top1 = premiumPick;
const top3 = smartCombo;
const top5 = highCombo;
