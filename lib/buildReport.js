export async function buildEliteReport(lang = "ro", apiKey) {
  ...
  // ─── CORNERE — medie din ultimele 10 meciuri ──────────────────────────────
  ...
  async function analyzeMatch(match) {
    ...
    if (under25Score >= MIN_CONFIDENCE && poisson.expectedGoals <= 2.2) {
      picks.push(buildPick({
    ...
    const r1Score = combinedConfidence(
      poisson.firstHalfGoalPct, r1Classic, 65, avgSP
    );
    ...
    if (oneXScore >= MIN_CONFIDENCE && eloDiff >= 50 && homeForm >= awayForm) {
      picks.push(buildPick({
    ...
    if (x2Score >= MIN_CONFIDENCE && eloDiff <= -50 && awayForm >= homeForm) {
      picks.push(buildPick({
    ...
    // ── CORNERE DINAMIC — FILTRU DUBLU + 10 MECIURI ───────────────────────
    try {
      ...
      if (homeCorners !== null && awayCorners !== null) {
        ...
        // FILTRU DUBLU — ambele echipe trebuie să confirme individual
